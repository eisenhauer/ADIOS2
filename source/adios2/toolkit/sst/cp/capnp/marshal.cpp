        ...
        else
        {
            ...
            for (int j = 0; j < 1; j++)
            {
                ...
                if (MetaEntry->Offsets)
                {
                    ...
                    for (int k = 0; k < MetaEntry->Dims; k++)
                    {
                        start.set(k, MetaEntry->Offsets[k]);
                    }
                }
                auto count = bi.initCount(MetaEntry->Dims);
                for (int k = 0; k < MetaEntry->Dims; k++)
                {
                    count.set(k, MetaEntry->Count[k]);
                }
            }
        }
            ...
            for (int i = 0; i < blocks.size(); i++)
            {
                ...
                if (block.hasStart())
                {
                    ...
                    if ((Dims > 1) && (Stream->WriterConfigParams->IsRowMajor !=
                                       Stream->ConfigParams->IsRowMajor))
                    {
                        ReverseDimensions(VarRec->PerWriterStart[WriterRank],
                                          VarRec->DimCount);
                    }
                }
                else
                ...
                if (block.hasCount())
                {
                    ...
                    if ((Dims > 1) && (Stream->WriterConfigParams->IsRowMajor !=
                                       Stream->ConfigParams->IsRowMajor))
                    {
                        ReverseDimensions(VarRec->PerWriterCounts[WriterRank],
                                          Dims);
                    }
                }
                else
